...
<h5>
The data for birth ratio by region comes from the General statistics office of Vietnam, the dataset is uploaded to Github, then retrieved via downloading. The first empty row was removed, extracting headers and data, and naming it "birthDataByRegion". We then rename the headers to make it displays the year of retrieval.\
</h5>
...
```{r}
u <- "https://geodata.ucdavis.edu/gadm/gadm3.6/Rsp/gadm36_VNM_1_sp.rds"
download.file(u, "vnm_geospatial.rds", mode = "wb")
vietnam_prov <- readRDS("vnm_geospatial.rds")
vietnam_prov_df <- vietnam_prov %>% fortify(region = "NAME_1")
vietnam_prov_df %>% 
  mutate(prov_names = stri_trans_general(id, "Latin-ASCII")) -> vietnam_prov_df
```
:::
<h5>
...
```{r fig.align = 'center'}
#Processing data
url <- "https://github.com/lamngue/Vietnam-Gender-Statistics/blob/3c80f45d1d7a46106fa4c01a595fd1338ada5c5f/Percentage%20of%20Literate%20Population.xlsx?raw=true"
...
```{r fig.align = 'center'}
#Processing data
url <- "https://github.com/lamngue/Vietnam-Gender-Statistics/blob/1d727a73b7ab8064b8564d97a46d91d498bb5e54/Fertility%20rate.xlsx?raw=true"
...
```
<h5>
  The fertility rates of South East and Red River Delta regions are among the lowest in the nation, which aligns with the preconceived notion - "In smaller families, you’re less likely to have a son by chance" (source). 
</h5>
```{r echo=FALSE, fig.align = 'center'}
url <- "https://github.com/lamngue/Vietnam-Gender-Statistics/blob/main/Infant%20mortality%20Rate%20by%20Region.xlsx?raw=true"
...
``` 
<h5>
From the matrix, the smallest correlation is 0.33, also some variables seem to have a weak, positive, linear relationship with the graduation rate. All correlations between the explanatory variables are smaller than 0.7. Therefore, we can conclude that there’s no multicollinearity issue in this model.
</h5>
